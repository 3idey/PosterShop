<footer class="bg-white/90 border-t border-indigo-100 mt-16">
    <div class="container mx-auto px-6 py-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        <div>
            <div class="flex items-center gap-3">
                <img src="{{ \Illuminate\Support\Facades\Vite::asset('resources/images/fulllogo.png') }}" alt="PosterShop Logo" class="w-10 h-10 rounded-full shadow" onerror="this.onerror=null;this.src='{{ \Illuminate\Support\Facades\Vite::asset('resources/images/icon.jpg') }}';">
                <span class="text-xl font-bold text-indigo-700">PosterShop</span>
            </div>
            <p class="mt-4 text-indigo-500/80">Discover, collect, and share creative posters from around the world.</p>
        </div>
        <div>
        ...
        <div>
            ...
            <p class="text-indigo-500/80">Get updates on new posters and collections.</p>
            <form class="mt-3 flex gap-2">
                <input type="email" placeholder="you@example.com" class="flex-1 rounded-xl border border-indigo-200 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-400">
                <button type="button" class="rounded-xl bg-indigo-600 text-white px-4 py-2 hover:bg-indigo-700">Subscribe</button>
            </form>
        </div>
    </div>
